...
datadir=$HOME/data/giga
if [ ! -d $datadir ]; then
  echo Directory not exists: $datadir
  exit -1
fi

# base url for downloads.
giga_repo=git@github.com:SpeechColab/GigaSpeech.git
...
if [ ${stage} -le -1 ] && [ ${stop_stage} -ge -1 ]; then
    echo "stage -1: Clone SpeechColab repo"
    if [ ! -d GigaSpeech ]; then
      git clone $giga_repo
      cp ~/.ossutilconfig GigaSpeech/SAFEBOX/aliyun_ossutil.cfg
      sed -i.bak "s#~/GigaSpeech#$datadir#" GigaSpeech/env_vars.sh
    fi
fi
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    cd GigaSpeech
    . toolkits/kaldi/dowload_analyze.sh
    cd -
    . GigaSpeech/env_vars.sh
    GigaSpeech/toolkits/kaldi/gigaspeech_data_prep.sh data
fi
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    [ -d data/${train_set}_org ] || cp -Rf data/${train_set} data/${train_set}_org
    [ -d data/${train_dev}_org ] || cp -Rf data/${train_dev} data/${train_dev}_org
